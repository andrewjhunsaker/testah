...
**Commits:** Direct to `main` until the repo has a GitHub remote (Task 14); branch + PR discipline starts after that.
...
`targets.yaml` starts with `home` (`/`) and `help` (`/help`) — both public,
no auth. Add/replace pages if you want different coverage for the first loop.
