...
- Substrate fix round (commit d0b8f9e) after the Tasks 5–7 review: drift hash
  keeps indentation/blank lines (trailing-whitespace-only normalization) and
  now includes `page.json` (selector changes count as drift); mailbox `new`
  is sticky (never downgrades to `changed` before the Author consumes it);
  flake ids include `projectName`; `update_history` is idempotent per run-id;
  `over_threshold` scopes to tests present in the current report; atomic +
  utf-8 JSON writes; CI hardened (push→master only, concurrency-cancel,
  `permissions: contents: read`, 15-min timeouts). Unit suite: 14 tests.
- Agent-file audit round: added a shared "Conventions (all agents)" block in
  scout.md (repo-root CWD, ISO dates, `auth:` value = role key, MCP fallback
  naming, PR-without-remote behavior); Scout's redirect check compares
  normalized paths and discards wrong-page artifacts; judge pass records
  `judge_model` in meta.json; Author spawns the Reviewer as a FRESH subagent
  and documents `gh run download` for CI reports + role-locator derivation;
  Reviewer template no longer embeds its own instructions and pins the
  flake-tracker id format; Steward defines ticket-slug naming (title slug),
  the `approved` flip (human-owned) and `direct`-mode lifecycle, and the
  Linear project name (**testah** — stop and ask if missing). Added
  `.mcp.json` (chrome-devtools-mcp + Linear via mcp-remote).
